...
class FamilyManagerWindow(forms.WPFWindow):
    ...
    def populate_list(self):
        ...
        for fam in sorted(families, key=lambda x: x.Name):
            # IsEditable garante que não estamos tentando editar famílias de sistema (Paredes, Pisos, etc.)
            if fam.IsEditable: 
    ...
    # --- LÓGICA DE CÁLCULO ATUALIZADA (FILTRO DE SISTEMA) ---
    def OnCalculateSizeClick(self, sender, e):
        selected = self.get_selected_items()
        if not selected:
            forms.alert("Selecione famílias para analisar.")
            return
        ...
        try:
            with forms.ProgressBar(title="Analisando arquivos RFA...", cancellable=True) as pb:
                for idx, item in enumerate(selected):
                    if pb.cancelled: break

                    # Só tenta abrir se for família carregável e não for "In-Place"
                    if item.FamilyObj.IsEditable and not item.FamilyObj.IsInPlace:
                        if item.size_mb > 0:
                        ...
                        item.SizeText = "Família de Sistema (Sem RFA)"
